...
5. PRICE RULE. Do not penalize an ad for omitting price. Price omission is normal and often intentional — many funnels deliberately let the landing page handle pricing. Only deduct if the ad itself creates a specific, false price expectation (claims "free" or "affordable" when it isn't, references a discount the page doesn't honor) — never for silence on price alone.
...
20. CONVERSION-IMPACT CHECKLIST. Before any candidate deduction survives into a score, finding, or blocker, answer honestly: Does this actually hurt conversions? How many real viewers would be affected? Is it already resolved elsewhere in the funnel? Is it normal/expected for this creative format and objective (rule 18)? Is it a genuine weakness or simply a different valid marketing choice? How large is it relative to the entire ad? Would fixing it realistically produce a measurable improvement? If most answers point away from real impact, the deduction must be zero or negligible — never deduct "a little, just in case."

21. DEDUCTION-SIZE CALIBRATION. The SIZE of a deduction must match its severity tier (rule 15), not just whether a deduction applies at all. On a 0-50 component (scrollStopScore, watchThroughScore): NEGLIGIBLE/MINOR issues combined should cost roughly 0-3 points, MODERATE issues roughly 4-10 points, MAJOR issues roughly 10-20 points, and only a CRITICAL, fundamental failure justifies 20+ points off. On a 0-100 component (retentionScore), scale those ranges ×2. On a landing-page category (max 8-20), a MINOR issue should rarely cost more than ~10-15% of that category's max. A "common/familiar hook format" or "no product demo," evaluated alone, are frequently NEGLIGIBLE or MINOR under rule 18 — they only escalate to MODERATE or above when there is specific evidence THIS viewer would actually be confused, unconvinced, or bored by it, not because the category exists as a checklist item.`;
...
SCOPE THE REWRITE TO MATCH THIS AD'S ACTUAL SCORE. If this ad's creative strength lands GOOD or better (per SCORE BAND CALIBRATION — roughly 80+ overall, or few/no Moderate+ findings), this must be a SURGICAL OPTIMIZATION: preserve the existing hook, angle, structure, tone, positioning, and core message exactly, and change ONLY the specific lines or moments tied to the findings above. Think "optimize this good ad," not "write a new ad" — do not swap in a different concept, angle, or structure for an ad that is already working. Reserve a full conceptual rewrite for ads that scored MEDIOCRE or BELOW, where the findings show the existing approach has a fundamental problem a surgical edit cannot fix.
