...
export function HookLibraryEmpty() {
  return (
    <div className="dash-card relative mt-10 overflow-hidden rounded-[28px] p-10 text-center">
      <div className="pointer-events-none absolute inset-0 opacity-30 blur-[2px]">
        <div className="grid gap-3 p-6 md:grid-cols-3">
          {PREVIEW_HOOKS.map((p, i) => (
            <div key={i} className="app-inset rounded-2xl p-4 text-left">
              <p className="text-sm font-medium text-white/55">&ldquo;{p.hook}&rdquo;</p>
              <div className="mt-3 flex gap-2">
                <span className="app-chip px-2 py-0.5 text-[10px]">{p.platform}</span>
                <span className="rounded-full border border-accent/25 bg-accent/10 px-2 py-0.5 text-[10px] text-accent-tertiary">
                  {p.tag}
                </span>
              </div>
            </div>
      ...
      <div className="relative">
        <div className="icon-badge mx-auto flex h-14 w-14 items-center justify-center rounded-2xl border border-accent/20 bg-accent/[0.12]">
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" aria-hidden>
            <path d="M4 8H20M4 12H16M4 16H12" stroke="#a78bfa" strokeWidth="1.5" strokeLinecap="round" />
          </svg>
        </div>
        <h2 className="mt-5 font-display text-2xl font-semibold tracking-[-0.04em] text-white">
