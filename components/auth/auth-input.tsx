...
type AuthInputProps = InputHTMLAttributes<HTMLInputElement> & {
  label: string;
  error?: string;
  /** `app` uses dark glass input-field styles inside the dashboard shell */
  tone?: "auth" | "app";
};
...
export function AuthInput({
  ...
  return (
    <div className="space-y-1.5">
      <label
        htmlFor={inputId}
        className={`block text-sm font-semibold tracking-[-0.01em] ${
          tone === "app" ? "text-white/75" : "text-text-primary"
        }`}
      >
        {label}
      </label>
